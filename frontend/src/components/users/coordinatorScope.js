 ...
 * el alcance por programa exige elegir uno.
 */
export function refineCoordinatorScope(values, ctx) {
  ...
  if (values.coordinator_scope === 'PROGRAM' && !values.coordinator_program) {
    ctx.addIssue({
      code: 'custom',
      path: ['coordinator_program'],
      message: 'Selecciona el programa que coordina',
    })
  }
}
 ...
 * nunca viaja con programa. Evita que quede basura al cambiar de rol.
 */
export function coordinatorScopePayload(values) {
  if (values.role !== 'COORDINATOR') {
    return { coordinator_scope: '', coordinator_program: null }
  }

  return {
    coordinator_scope: values.coordinator_scope,
    coordinator_program:
      values.coordinator_scope === 'PROGRAM' ? values.coordinator_program : null,
  }
}
