...
def process_payment_ocr(self, payment_id):
    ...
    try:
        ...
        mime_type = (
            'application/pdf'
            if payment.receipt_file_type == 'pdf'
            else 'image/jpeg'
        )
...
def send_payment_status_notification(self, payment_id, new_status):
    ...
    try:
        ...
        payment    = Payment.objects.select_related('bootcamper', 'program').get(id=payment_id)
        ...
        logger.exception('Error sending payment status notification for %s.', payment_id)
        raise self.retry(exc=exc)
