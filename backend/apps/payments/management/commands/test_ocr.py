...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('')
        self.stdout.write(
            self.style.WARNING(f'  Overall confidence: {overall:.2f} {_conf_bar(overall)}')
        )
        ...
        if raw.strip():
            ...
            self.stdout.write(self.style.ERROR('  (empty — check Tesseract install and language pack)'))
