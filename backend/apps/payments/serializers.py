...
class PaymentApproveSerializer(serializers.Serializer):
    ...
    confirmed_bank_name      = serializers.CharField(max_length=200, required=False, allow_blank=True)
    confirmed_transaction_id = serializers.CharField(max_length=100, required=False, allow_blank=True)
...
class PaymentRejectSerializer(serializers.Serializer):
    ...
    def validate_rejection_reason(self, value):
        if not value.strip():
            raise serializers.ValidationError('El motivo de rechazo no puede estar vacío.')
        return value
